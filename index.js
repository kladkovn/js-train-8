...
function removeLastElement(arr) {
  ...
  arr.pop();
  return arr;// Перевіряємо, чи масив не є порожнім, якщо порожній повертаємо  undefined
...
function customFill(arr, value, start = 0, end = arr.length) {
 ...
 if(!Array.isArray(arr)) {
  return false;// Перевірка вхідних параметрів
 }// якщо arr не масив повертаємо false
...
function customSome(arr, condition) {
 ...
 if(typeof condition !== 'function'){
  ...
  const result = arr.some(condition);// Використовуємо метод `some` для перевірки умови хоча б для одного елементу масиву
...
function customAt(arr, index) {
  ...
  const element = arr.at(index);// Отримуємо елемент масиву за заданим індексом
  if(typeof element === 'string'){
    console.log('element is string');
  } else if(typeof element === 'object'){
    console.log('element is object');   // перевіремо якого типу елемент ми отримали,якщо рядок виведом в консоль -Елемент є рядком,числом - Елемент є числом, об`єктом -Елемент є обєктом
